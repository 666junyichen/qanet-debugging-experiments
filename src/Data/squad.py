...
def sanity_check_cache(args):
    ...
    if train_data["ids"].shape[0] <= 0:
        raise ValueError("train.npz has zero samples.")

    if (train_data["y1s"] > train_data["y2s"]).any():
        raise ValueError("Found y1 > y2 in train.npz (span start > end).")

    # Optional shape consistency checks for train
    n_train = train_data["ids"].shape[0]
    for k in required_keys:
        if train_data[k].shape[0] != n_train:
            raise ValueError(
                f"Inconsistent first dimension in train.npz: "
                f"{k} has {train_data[k].shape[0]} samples, expected {n_train}."
            )

    # Check dev cache
    dev_data = np.load(args.dev_npz)

    for k in required_keys:
        if k not in dev_data:
            raise KeyError(f"{args.dev_npz} missing key: {k}")

    if dev_data["ids"].shape[0] <= 0:
        raise ValueError("dev.npz has zero samples.")

    if (dev_data["y1s"] > dev_data["y2s"]).any():
        raise ValueError("Found y1 > y2 in dev.npz (span start > end).")

    # Optional shape consistency checks for dev
    n_dev = dev_data["ids"].shape[0]
    for k in required_keys:
        if dev_data[k].shape[0] != n_dev:
            raise ValueError(
                f"Inconsistent first dimension in dev.npz: "
                f"{k} has {dev_data[k].shape[0]} samples, expected {n_dev}."
            )
