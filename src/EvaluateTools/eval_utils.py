...
def run_eval(model, dataset, eval_file, num_batches, batch_size,
    ...
    for Cwid, Ccid, Qwid, Qcid, y1, y2, ids in tqdm(
        ...
        for i in range(p1.size(0)):  # each sample in batch
            score = p1[i].unsqueeze(1) + p2[i].unsqueeze(0)   # [L, L]
            L = score.size(0)

            # keep only spans with start <= end
            mask = torch.triu(torch.ones(L, L, device=score.device), diagonal=0).bool()
            score = score.masked_fill(~mask, -1e30)

            flat_idx = torch.argmax(score)
            start = (flat_idx // L).item()
            end = (flat_idx % L).item()

            yp1_list.append(start)
            yp2_list.append(end)

        answer_dict_, _ = convert_tokens(eval_file, ids.tolist(), yp1_list, yp2_list)
